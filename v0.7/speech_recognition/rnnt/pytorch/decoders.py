...
class ScriptGreedyDecoder(torch.nn.Module):
    ...
    def __init__(self, blank_index, model, max_symbols_per_step=30):
        ...
        self.eval()
        self._model = model
        self._blank_id = torch.empty(1, dtype=torch.int)
        self._blank_id.fill_(blank_index)
#        self._blank_id = blank_index
        self._SOS = torch.empty(1, dtype=torch.int)
        self._SOS.fill_(-1)
#        self._SOS = -1
    ...
    def _greedy_decode(self, x: torch.Tensor, out_len: torch.Tensor) -> List[torch.Tensor]:
        hidden: Optional[Tuple[torch.Tensor, torch.Tensor]] = None
        label: List[torch.Tensor] = []
        for time_idx in range(out_len.item()):
